...
updateUIByRole();
    ...
    // Gửi request với user_id và role
    QJsonObject json;
    json["user_id"] = UserData::instance().getUserId();
    json["role"] = UserData::instance().getRole();
    
    QJsonDocument doc(json);
    QString dataString = QString("CONTROL GET_MY_CLASSES\n%1").arg(QString(doc.toJson(QJsonDocument::Compact)));
...
void ClassList::onReadyRead()
{
    ...
    if (responseString.startsWith("DATA JSON GET_MY_CLASSES")) {
...
void ClassList::on_logoutButton_clicked()
{
    UserData::instance().reset();
    emit logout();
}

void ClassList::updateUIByRole()
{
