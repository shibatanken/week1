...
void handle_review_appeal(int client_socket, ControlMessage *msg)
{
    ...
    int result = review_appeal(appeal_id, teacher_id, status, response_text, teacher_comment);
    char response[2048];
    char timestamp[50];
    time_t now = time(NULL);
    strftime(timestamp, sizeof(timestamp), "%Y-%m-%dT%H:%M:%S", localtime(&now));
...
void handle_get_appeal_history_for_admin(int client_socket, ControlMessage *msg)
{
    char *result = get_appeal_history_for_admin();
    char *response;

    if (result == NULL) {
        response = strdup("DATA JSON APPEAL_HISTORY\n{\"data\": []}");
    } else {
        size_t response_size = strlen(result) + 64;
        response = (char *)malloc(response_size);
        snprintf(response, response_size, "DATA JSON APPEAL_HISTORY\n{\"data\": %s}", result);
        free(result);
    }
