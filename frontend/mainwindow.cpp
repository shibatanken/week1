...
void MainWindow::showExamResult(int submissionId) {
    qDebug() << "=== MainWindow::showExamResult ===";
    qDebug() << "submissionId:" << submissionId;
    
    // Hiển thị widget trước, sau đó load dữ liệu
    ui->stackedWidget->setCurrentWidget(examTakingForm);
    qDebug() << "Set examTakingForm, current widget index:" << ui->stackedWidget->currentIndex();
    
    // Load kết quả (sẽ tự hiển thị loading state và result page)
    examTakingForm->loadExamResult(submissionId);
