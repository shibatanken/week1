...
char *get_my_appeals(int user_id)
{
    ...
    snprintf(query, sizeof(query),
             ...
             "a.reason, a.status, a.teacher_response, a.score_adjustment, "
    ...
    while ((row = mysql_fetch_row(res))) {
        ...
        cJSON_AddNumberToObject(obj, "score_adjustment", row[7] ? atof(row[7]) : 0);
        cJSON_AddStringToObject(obj, "created_at", row[8]);
        cJSON_AddStringToObject(obj, "resolved_at", row[9] ? row[9] : "");
        cJSON_AddStringToObject(obj, "exam_name", row[10]);
...
char *get_appeals_for_teacher(int teacher_id)
{
    ...
    while ((row = mysql_fetch_row(res))) {
        ...
        cJSON_AddStringToObject(obj, "resolved_at", row[9] ? row[9] : "");
        cJSON_AddStringToObject(obj, "exam_name", row[10]);
        cJSON_AddStringToObject(obj, "student_name", row[11]);
        cJSON_AddStringToObject(obj, "student_email", row[12]);
        cJSON_AddItemToArray(json_array, obj);
    }
...
int review_appeal(int appeal_id, const char *status, const char *response, double score_adj)
{
    ...
    snprintf(query, sizeof(query),
             ...
             "score_adjustment = %.2f, resolved_at = NOW() WHERE id = %d",
